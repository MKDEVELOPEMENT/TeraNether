...
import static TeraNether.TNWorldGenerator.TERANETHER_DEPTH;

@RegisterSystem(RegisterMode.CLIENT)
public class TNClientSystem extends BaseComponentSystem implements UpdateSubscriberSystem {
    ...
    public void onDeath(BeforeDestroyEvent event, EntityRef entity, CharacterComponent characterComponent, LocationComponent locationComponent) {
        ...
        for (int i = 0; i < InventoryUtils.getSlotCount(character); i++) {
            if (InventoryUtils.getItemAt(character, i).hasComponent(TNResurrectComponent.class)) {
                resurrect = true;
        ...
        if (resurrect) {
            ...
            if (spawnPos != null) {
                ...
                character.send(new EnterTNEvent(client));
    ...
    @ReceiveEvent
    public void onEnterNether(EnterTNEvent event, EntityRef entity) {
        // temp-fix, not ideal
    ...
    private Vector3f findInfernoSpawn(Vector3f currentPos) {
        ...
        Region3i searchArea = Region3i.createFromCenterExtents(new Vector3i(currentPos.x(), -TERANETHER_DEPTH, currentPos.z()), searchRadius);
        Region worldRegion = world.getWorldData(searchArea);

        TNSurfaceHeightFacet surfaceHeightFacet = worldRegion.getFacet(TNSurfaceHeightFacet.class);
        TNCeilingHeightFacet ceilingHeightFacet = worldRegion.getFacet(TNCeilingHeightFacet.class);
